 ...
 *   description: Anagrafica clienti / fornitori - contatti
 */

/**
 * @swagger
 * /ba_cliforcon001/preview:
 *   get:
 *     summary: Preview di ba_cliforcon005
 *     description: |
 *       Restituisce una **anteprima paginata** dei contatti
 *       clienti / fornitori presenti in ba_cliforcon005.
...
router.get('/preview', async (req, res) => {
  try {
    ...
    const countResult = await pool.query(
      'SELECT COUNT(*) FROM ba_cliforcon005'
    );
    const totalRows = parseInt(countResult.rows[0].count, 10);
    const totalPages = Math.ceil(totalRows / limit);

    const { rows } = await pool.query(
      'SELECT * FROM ba_cliforcon005 ORDER BY cctipsog, cccodsog OFFSET $1 LIMIT $2',
      [offset, limit]
    );
...
/**
 * @swagger
 * /ba_cliforcon001/export:
 *   get:
 *     summary: Esporta ba_cliforcon005 in ZIP (JSON formattato, streaming)
 *     description: |
 *       Questa API esporta **tutti i record della tabella ba_cliforcon005**
 ...
 *
 *       Cosa ottieni:
 *       • Un file `ba_cliforcon005.zip`
 *       • All’interno trovi `ba_cliforcon005.json`
 *
 *       Come usarla:
...
router.get('/export', async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename="ba_cliforcon005.zip"'
    );
    ...
    const query = new QueryStream('SELECT * FROM ba_cliforcon005');
    const dbStream = client.query(query);

    archive.append(
      dbStream.pipe(prettyJsonStream()),
      { name: 'ba_cliforcon005.json' }
    );
