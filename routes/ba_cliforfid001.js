...
/**
 * @swagger
 * /ba_cliforfid001/preview:
 *   get:
 *     summary: Preview di ba_cliforfid005
 *     description: |
 *       Restituisce una **anteprima paginata** dei dati di fidelizzazione clienti.
...
router.get('/preview', async (req, res) => {
  try {
    ...
    const countResult = await pool.query(
      'SELECT COUNT(*) FROM ba_cliforfid005'
    );
    const totalRows = parseInt(countResult.rows[0].count, 10);
    const totalPages = Math.ceil(totalRows / limit);

    const { rows } = await pool.query(
      'SELECT * FROM ba_cliforfid005 ORDER BY cftipsog, cfcodsog OFFSET $1 LIMIT $2',
      [offset, limit]
    );
...
/**
 * @swagger
 * /ba_cliforfid001/export:
 *   get:
 *     summary: Esporta ba_cliforfid005 in ZIP (JSON formattato, streaming)
 *     description: |
 *       Questa API esporta **tutti i record della tabella ba_cliforfid005**
 ...
 *
 *       Cosa ottieni:
 *       • Un file `ba_cliforfid005.zip`
 *       • All’interno trovi `ba_cliforfid005.json`
 *
 *       Come usarla:
...
router.get('/export', async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename="ba_cliforfid005.zip"'
    );
    ...
    const query = new QueryStream('SELECT * FROM ba_cliforfid005');
    const dbStream = client.query(query);

    archive.append(
      dbStream.pipe(prettyJsonStream()),
      { name: 'ba_cliforfid005.json' }
    );
