...
/**
 * @swagger
 * /ba_umiart001/preview:
 *   get:
 *     summary: Preview di ba_umiart005 
 *     description: |
 *       Restituisce una **anteprima paginata** delle unità di misura articoli.
...
router.get('/preview', async (req, res) => {
  try {
    ...
    const countResult = await pool.query('SELECT COUNT(*) FROM ba_umiart005');
    const totalRows = parseInt(countResult.rows[0].count, 10);
    const totalPages = Math.ceil(totalRows / limit);

    const { rows } = await pool.query(
      'SELECT * FROM ba_umiart005 ORDER BY uaflgart, uakeyart LIMIT $1 OFFSET $2',
      [limit, offset]
    );
 ...
 * /ba_umiart001/export:
 *   get:
 *     summary: Esporta ba_umiart005 in ZIP (JSON formattato, streaming)
 *     description: |
 *       Questa API esporta **tutti i record della tabella ba_umiart005**
 ...
 *
 *       Cosa ottieni:
 *       • Un file `ba_umiart005.zip`
 *       • All’interno trovi `ba_umiart005.json`
 *
 *       Come usarla:
...
router.get('/export', async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename="ba_umiart005.zip"'
    );
    ...
    const query = new QueryStream('SELECT * FROM ba_umiart005');
    const dbStream = client.query(query);

    // ✅ usa prettyJsonStream per JSON leggibile
    archive.append(dbStream.pipe(prettyJsonStream()), { name: 'ba_umiart005.json' });
